...
int main(){
    ...
    cout<<"Velocidad del puerto serial: \x1b[32m"<<baud<<"\x1b[0m"<<endl;
    // Abrir puerto serial:
    ...
    long unsigned int lineas = 1 ; // Contador de lineas
...
string rectaXY( double *vector1, double *vector2 ){
    ...
    double ty = vector2[1] - vector1[1] ;
    double tx = vector2[0] - vector1[0] ;

    string comando ;
    double x = vector1[0] ;
    double y = vector1[1] ;

    for( double t = 0 ; t <= 1 ; t = t + 0.0000001 ){

        if( abs(t*tx + vector1[0] - x) >= deltaX  ){
            if( x < vector2[0] ){
                comando = comando + 'X' ;
            }else if( x > vector2[0] ){
                comando = comando + 'x' ;
        ...
        if( abs(t*ty + vector1[1] - y) >= deltaY ){
            if( y < vector2[1] ){
                comando = comando + 'Y' ;
            }else if( y > vector2[1] ){
                comando = comando + 'y' ;
...
string rectaZ( double *vector1, double *vector2 ){
    ...
    double z1 = vector1[2] ;
    vector1[2] = vector2[2] ;
    return string( abs( vector2[2]-z1 )/deltaZ, z ) ;
}
string circHor( double *vector1, double *vector2, double *centro ) {
    ...
    for( double k = teta1 ; k >= teta2 ; k = k-2*asin( hypot(deltaX,deltaY)/(2*r))/1000  ){
